        ...
        <div class="registration-box" :style="boxStyle()">
            ...
            <v-form class="registration-form" @submit.prevent>
                <v-responsive class="registration-input-bar">
                    <v-text-field class="registration-input-handler" :rules="[required]" ref="NameInput" label="Student Name"
                        variant="outlined"></v-text-field>

                    <div class="occupation-and-gender-container">
                        <div class="registration-occupation">
                            <v-text-field class="registration-input-handler" :rules="[required]" ref="EducationInput"
                                label="Education" variant="outlined"></v-text-field>
                        </div>
...
import { ref } from "vue";
import { API, CURRENT, SETSTUDENT, SETTOKEN } from "../main"
...
function boxStyle() {
    return {
        maxHeight: error.value ? '810px' : '750px',
    }
...
function login() {
    let checkUser = async () => {
        error.value = "";
        if (!emailInput.value.value || !passwordInput.value.value || !GenderInput.value || !EducationInput.value.value || !NameInput.value.value) return;
        loading.value = true;
        ...
        try {
            const response = await fetch(API + '/create/user', {
                ...
                body: JSON.stringify({
                    ...
                    education: EducationInput.value.value,
